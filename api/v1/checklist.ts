...
import { addReferralUrls, intParam, logApi, optionalParam, referralSource, requestUrl, requireGet, retagMcpReferral, sendJson } from '../../src/http-json.js';

export default function handler(req: IncomingMessage, res: ServerResponse): void {
  if (!requireGet(req, res)) return;
  const url = requestUrl(req);
  const concerns = url.searchParams.getAll('concern').map((value) => value.trim()).filter(Boolean).slice(0, 10);
  const profile = {
  ...
  const checklist = buildBuyerChecklist(profile, intParam(url, 'limit', 12, 4, 20));
